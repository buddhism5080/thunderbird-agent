...
```
Agent runtime / CLI <--HTTP POST--> Thunderbird Extension (port 8765)
```

- **packages/cli/thunderbird-agent.cjs**: Node.js process, translates stdio JSON-RPC ↔ HTTP
...
- **API file**: `extension/agent_server/api.js` (~1920 lines, monolithic — all tool defs + handlers)
...
```json
{
  "experiment_apis": {
    "agentServer": {
      "schema": "agent_server/schema.json",
      "parent": {
        "scopes": ["addon_parent"],
        "paths": [["agentServer"]],
        "script": "agent_server/api.js"
      }
    }
  },
  "permissions": [
...
### Manual Verification
1. Create a filter via the local agent tool
...
4. Modify the filter via the local agent transport → verify changes in Thunderbird UI
