...
describe("Tool metadata: tools/list stripping", () => {
   ...
   * Input tools have internal fields; output should only have public fields.
   */
  function stripForToolsList(toolsArray) {
    return toolsArray.map(({ name, description, inputSchema }) => ({ name, description, inputSchema }));
  }

  const ALLOWED_PUBLIC_FIELDS = new Set(["name", "description", "inputSchema"]);
  ...
  it("output contains only public fields (allowlist check)", () => {
    ...
    for (const tool of stripped) {
      for (const key of Object.keys(tool)) {
        assert.ok(ALLOWED_PUBLIC_FIELDS.has(key), `Unexpected field "${key}" leaked in tools/list output`);
      }
    }
  });
...
describe("Structural: test data matches production source", () => {
  it("ALL_TOOLS count matches tool definitions in api.js", () => {
    const apiPath = path.join(__dirname, "..", "extension", "agent_server", "api.js");
