...
function makeTempRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'tb-conn-discovery-'));
}
...
describe('Connection discovery', () => {
  ...
  it('env var override takes priority', () => {
    const options = makeTestOptions(root, {
      env: { THUNDERBIRD_AGENT_CONNECTION_FILE: path.join(root, 'env', 'connection.json') },
    });

    writeConnectionFile(path.join(root, 'tmp', 'thunderbird-agent', 'connection.json'), {
      port: 20001,
      token: 'native-token',
    });
    writeConnectionFile(options.env.THUNDERBIRD_AGENT_CONNECTION_FILE, {
      port: 20002,
      token: 'env-token',
    });
  ...
  it('snap detection works from a mocked /proc tree', () => {
    ...
    writeConnectionFile(path.join(snapTmpDir, 'thunderbird-agent', 'connection.json'), {
      port: 20003,
      token: 'snap-token',
    });
  ...
  it('snap detection ignores decoy processes with thunderbird only as a file arg', () => {
    ...
    writeConnectionFile(path.join(decoyTmpDir, 'thunderbird-agent', 'connection.json'), {
      port: 29999,
  ...
  it('macOS scan finds current uid files and ignores other owners', () => {
    ...
    const ownedConnFile = path.join(darwinRoot, 'aa', 'bb', 'T', 'thunderbird-agent', 'connection.json');
    const foreignConnFile = path.join(darwinRoot, 'cc', 'dd', 'T', 'thunderbird-agent', 'connection.json');
  ...
  it('reports useful discovery failures', () => {
    const options = makeTestOptions(root, {
      env: { THUNDERBIRD_AGENT_CONNECTION_FILE: path.join(root, 'missing', 'connection.json') },
      platform: 'linux',
    });

    assert.equal(readConnectionInfo(options), null);
    assert.match(buildConnectionDiscoveryErrorMessage(), /THUNDERBIRD_AGENT_CONNECTION_FILE/);
