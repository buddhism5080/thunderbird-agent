...
/**
 * Thunderbird Agent Server Extension
 * Exposes email, calendar, and contacts to local AI agents over HTTP.
 *
 * Architecture: agent runtime / CLI <-> local HTTP server <-> Thunderbird (port 8765)
 *
...
const AGENT_MAX_PORT_ATTEMPTS = 10;
...
const PREF_ALLOWED_ACCOUNTS = "extensions.thunderbird-agent.allowedAccounts";
const PREF_DISABLED_TOOLS = "extensions.thunderbird-agent.disabledTools";
const PREF_BLOCK_SKIPREVIEW = "extensions.thunderbird-agent.blockSkipReview";
...
var agentServer = class extends ExtensionCommon.ExtensionAPI {
  getAPI(context) {
    const extensionRoot = context.extension.rootURI;
    const resourceName = "thunderbird-agent";
    ...
    const tools = [
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            saveAttachments: { type: "boolean", description: "If true, save attachments to <OS temp dir>/thunderbird-agent/<messageId>/ and include filePath in response (default: false)" },
      ...
      },
      {
        ...
        description: "Get the current account access control list. Shows which accounts the agent server can access. Account access is configured by the user in the extension settings page (Tools > Add-ons > Thunderbird Agent > Options) and cannot be changed from external agent calls.",
        inputSchema: { type: "object", properties: {}, required: [] },
      },
    ];
    ...
    if (toolErrors.length > 0) {
      console.error("thunderbird-agent: Tool metadata validation failed:\n  " + toolErrors.join("\n  "));
    }
    ...
    return {
      agentServer: {
        start: async function() {
          ...
          try {
            ...
            const { HttpServer } = ChromeUtils.importESModule(
              "resource://thunderbird-agent/httpd.sys.mjs?" + Date.now()
            );
             ...
             * so the CLI and agent runtimes can discover how to connect.
             * File: <TmpD>/thunderbird-agent/connection.json
             */
            function writeConnectionInfo(port, token) {
              const tmpDir = Services.dirsvc.get("TmpD", Ci.nsIFile);
              tmpDir.append("thunderbird-agent");
              if (!tmpDir.exists()) {
                tmpDir.create(Ci.nsIFile.DIRECTORY_TYPE, 0o700);
              } else if (tmpDir.isSymlink()) {
                throw new Error("thunderbird-agent tmp directory is a symlink — refusing to write connection info");
              }
            ...
            function getAllowedAccountIds() {
              try {
                ...
                if (!Array.isArray(parsed)) {
                  console.error("thunderbird-agent: allowed accounts pref is not an array, blocking all accounts");
                  return ["__invalid__"];
                }
                return parsed;
              } catch (e) {
                // Fail closed: corrupt pref means block all accounts, not allow all
                console.error("thunderbird-agent: failed to parse allowed accounts pref, blocking all accounts:", e);
                return ["__invalid__"];
              }
            }
            ...
            function getDisabledTools() {
              try {
                ...
                if (!Array.isArray(parsed) || !parsed.every(v => typeof v === "string")) {
                  console.error("thunderbird-agent: disabled tools pref is invalid, disabling all tools");
                  return ["__all__"];
                }
                return parsed;
              } catch (e) {
                console.error("thunderbird-agent: failed to parse disabled tools pref, disabling all tools:", e);
                return ["__all__"];
              }
            }
             ...
             *     to a temp file under <TmpD>/thunderbird-agent/attachments/
              ...
              try {
	            ...
	            function getMessage(messageId, folderPath, saveAttachments, bodyFormat, rawSource) {
                  ...
                  MsgHdrToMimeMessage(msgHdr, null, (aMsgHdr, aMimeMsg) => {
                    ...
                    if (!body) {
                      const fallbackContext = `thunderbird-agent: raw singlepart body fallback (${msgHdr.messageId})`;
                      let rawStream = null;
                      try {
            ...
            function deleteMessages(messageIds, folderPath) {
              try {
                // agent runtimes may send arrays as JSON strings
            ...
            function updateMessage(messageId, messageIds, folderPath, read, flagged, addTags, removeTags, moveTo, trash) {
              try {
                ...
                // Coerce boolean params (agent runtimes may send strings)
                ...
                // Coerce tag arrays (agent runtimes may send JSON strings)
            ...
            function createFilter(accountId, name, enabled, type, conditions, actions, insertAtIndex) {
              try {
                // Coerce arrays from agent runtime string serialization
             ...
             * agent runtimes may send "true"/"false" as strings for booleans,
                  ...
                  switch (method) {
                    case "initialize":
                      result = {
                        ...
                        capabilities: { tools: {} },
                        serverInfo: { name: "thunderbird-agent", version: "0.1.0" }
                      };
                      break;
                    case "resources/list":
                      result = { resources: [] };
                    ...
                    case "prompts/list":
                      ...
                      break;
                    case "tools/list":
                      // Strip internal metadata (group, crud, title) — only expose public fields
            ...
            let boundPort = null;
            for (let attempt = 0; attempt < AGENT_MAX_PORT_ATTEMPTS; attempt++) {
              const tryPort = AGENT_DEFAULT_PORT + attempt;
              try {
                server.start(tryPort);
                boundPort = tryPort;
                break;
              } catch (portErr) {
                if (attempt === AGENT_MAX_PORT_ATTEMPTS - 1) {
                  throw new Error(`Could not bind to any port in range ${AGENT_DEFAULT_PORT}-${tryPort}: ${portErr}`);
                }
            ...
            console.log(`Thunderbird Agent server listening on port ${boundPort}`);
        ...
        getServerInfo: async function() {
          ...
          try {
            const uri = Services.io.newURI("resource://thunderbird-agent/buildinfo.json");
  ...
  onShutdown(isAppShutdown) {
    ...
    resProto.setSubstitution("thunderbird-agent", null);
