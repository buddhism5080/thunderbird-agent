...
async function loadServerInfo() {
  try {
    const info = await browser.agentServer.getServerInfo();
    if (info.running) {
...
async function loadAccountAccess() {
  try {
    const data = await browser.agentServer.getAccountAccessConfig();
...
saveBtn.addEventListener("click", async () => {
  ...
  try {
    const result = await browser.agentServer.setAccountAccess(allowedIds);
    if (result.error) {
...
async function loadToolAccess() {
  try {
    const data = await browser.agentServer.getToolAccessConfig();
...
saveToolsBtn.addEventListener("click", async () => {
  ...
  try {
    const result = await browser.agentServer.setToolAccess(disabled);
    if (result.error) {
...
async function loadSkipReviewPref() {
  try {
    const { blockSkipReview } = await browser.agentServer.getBlockSkipReview();
...
saveSkipReviewBtn.addEventListener("click", async () => {
  ...
  try {
    const result = await browser.agentServer.setBlockSkipReview(blockSkipReviewCheckbox.checked);
    if (result.error) {
